...
{{
  ...
  "tags": ["タグ1", "タグ2", ...],
  "thumbnail": {{
    "team": "主役球団のキー (下の一覧から1つ)。複数球団を横断する比較ネタは『default』。該当なしなら null。",
    "headline": "サムネ最上部の黄色ボックスに入る短い予告 (全角6〜14文字)。default 時は空文字でよい",
    "main": "サムネ下半分のメインキャッチ1行目 (全角6〜14文字, 派手調)。マークアップ [文字|赤] で部分強調可",
    "sub": "メインキャッチ2行目 (全角6〜14文字, 省略可能なら空文字)。マークアップ [文字|赤] 可",
  ...
  "chapters": [
    ...
    {{
      "title": "球団非依存の比較トピック例",
      "narration": "...",
      "image_keywords": ["野球 球場"],
      "slide": {{
        "ranking": {{
          "rows": [
            {{"name": "選手A", "team": "ロ", "stats": ["148.1キロ", "12試合", "0.73", "0勝0敗8S"]}},
            {{"name": "選手B", "team": "デ", "stats": ["147.8キロ", "11試合", "1.74", "0勝0敗8S"]}},
            {{"name": "選手C", "team": "広", "stats": ["146.8キロ", "10試合", "2.70", "1勝1敗4S"]}}
          ]
        }},
        "footer": "NEXT➡もう1人の意外な選手も…!"
      }}
    }}
  ],
...
- footer: 球団 primary 色のバーで画面下に焼き付ける一言。次チャプター予告 (『NEXT➡…』) や締めに使う。不要なら空文字。
- ranking: 球団非依存の比較ネタ (各球団の○○ランキング等) で使う。
  指定時は bullets / right_box / highlight を無視して、中央パネルにランキング表を描画。
  rows[].name=選手名, rows[].team=球団略称 (ロ/デ/広/巨/神/中/ヤ/オ/ソ/楽/ハ/西), rows[].stats=複数の文字列カラム。
  3〜5 行程度。
  ranking を使うチャプターでは thumbnail.team を "default" にしてサムネも非依存テーマに切り替える。
